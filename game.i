...
# 1 "Normal.h" 1
# 21 "Normal.h"
extern const unsigned short NormalBitmap[128];
...
# 1 "ghostHunter.h" 1
# 21 "ghostHunter.h"
extern const unsigned short ghostHunterBitmap[128];


extern const unsigned short ghostHunterPal[256];
# 8 "game.c" 2
...
void initGame() {
    ...
    DMANow(3, NormalPal, ((unsigned short*) 0x05000000), 512 / 2);
...
void initHunter() {
    ...
    DMANow(3, &ghostHunterPal, ((unsigned short*) 0x05000000), 512 / 2);
}
...
void updatePlayer() {
    ...
    int speed = 2;
    if ((~(buttons) & ((1 << 5)))) { player.x -= speed; }
    if ((~(buttons) & ((1 << 4)))) { player.x += speed; }
    if ((~(buttons) & ((1 << 6)))) { player.y -= speed; }
    if ((~(buttons) & ((1 << 7)))) { player.y += speed; }
...
void updateHunter() {
    ...
    if (hunter.x > 240 - hunter.width)
        hunter.x = 240 - hunter.width;
    if (hunter.y < 0) hunter.y = 0;
    if (hunter.y > 160 - hunter.height)
        hunter.y = 160 - hunter.height;
}
